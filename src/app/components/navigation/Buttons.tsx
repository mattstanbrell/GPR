...
const ButtonImage = ({src, alt, width, height} : {src: string, alt: string, width: number, height: number}) => {
	return (
		<Image 
			src={ src }
			alt={ alt }
		...
		<>
		{
			!(isTitled) ? (
				<div title={ name } className='hover:cursor-pointer pl-5'>
					<ButtonImage src={ src } alt={ alt } width={ desktopIconSize } height={ desktopIconSize } />
				</div>
			) : (
				<div className='flex hover:cursor-pointer'>
					<div title={ name } className='hover:cursor-pointer'>
					<ButtonImage src={ src } alt={ alt } width={ mobileIconSize } height={ mobileIconSize } />
				</div>
...
const LinkedButton = (
	...
	{pageUrl: string, imageSrc: string, imageAlt: string, name: string, isTitled: boolean}
) => {
	return (
		<Link href={ pageUrl }>
...
const HomeButton = ({isTitled} : {isTitled: boolean}) => {
	...
	return <LinkedButton pageUrl={ url } imageSrc={ src } imageAlt={ alt } name={ name } isTitled={ isTitled } />
}
