...
const Button = ({ text, link, src, alt }: { text: string, link: string, src: string, alt: string }) => {
	...
	return (
		<Link href={link} className="text-3xl font-bold md:flex-none w-full flex-1 min-w-35 max-w-40 border-4 grid  h-47">
			<button className="cursor-pointer flex flex-col place-content-center">
				<Image
					...
					width={24}
					height={24}
					style={{filter: "saturate(100%) invert(22%) sepia(9%) saturate(5876%) hue-rotate(253deg) brightness(93%) contrast(89%)"}}
				/>
				<div className="grid  flex-1/2 self-center place-items-center text-3xl/10 text-(--hounslow-primary)">
					{firstLine}
